...
# Mis variables
# 76
  dataset[  , montos_prestamos  := mprestamos_personales + mprestamos_prendarios + mprestamos_hipotecarios]
# 14
  dataset[  , montos_tarjetas   := mtarjeta_visa_consumo + mtarjeta_master_consumo]
# 21
  dataset[  , montos_consumidos := montos_prestamos + montos_tarjetas]
  dataset[  , cantidad_seguros  := cseguro_vida + cseguro_auto + cseguro_vivienda + cseguro_accidentes_personales ]
  dataset[  , montos_cuentas    := mcuentas_saldo + mplazo_fijo_dolares + mplazo_fijo_pesos + minversion1_pesos + minversion1_dolares + minversion2 ]
  dataset[  , coef_comisiones   := mcomisiones / (montos_consumidos + montos_cuentas)]
...
Creo_Delta  <- function( dataset, NroLag )
{

dataset[ , paste0( cols_lagueables, "_lag", NroLag) := shift(.SD, NroLag, NA, "lag"), 
           by= numero_de_cliente, 
...
{
  dataset[ , paste0(vcol, "_delta", NroLag) := get(vcol)  - get(paste0( vcol, "_lag", NroLag))  ]
}
...
dir.create( "./exp/EntregaFinal/", showWarnings = FALSE )
setwd("./exp/EntregaFinal/")   #Establezco el Working Directory DEL EXPERIMENTO
...
#creo los campos lags de orden hasta 8

Creo_Delta ( dataset, 1 )
Creo_Delta ( dataset, 2 )
Creo_Delta ( dataset, 3 )
Creo_Delta ( dataset, 4 )
Creo_Delta ( dataset, 5 )
Creo_Delta ( dataset, 6 )
Creo_Delta ( dataset, 7 )
Creo_Delta ( dataset, 8 )
